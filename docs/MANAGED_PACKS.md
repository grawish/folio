...
The native core supports signed catalogs, resumable downloads, authenticated offline archives, deterministic bundle assembly, staged installation and exact repair from retained signed archives. **Resource-pack installation is not yet available in the desktop app.** The pack APIs are not connected to IPC or Settings, and no production signing key, public pack catalog or pack archive is configured. The remaining work below is part of the original release goal.
...
1. Add Settings-only catalog refresh, local import, download progress/cancel/resume, cache removal, installed-pack status and explicit backed-up compiler comparison. Pause conflicting operations and handle native close and renderer reload. Retain archives as part of the coordinated installation flow.
2. Configure production public trust roots, catalog hosting, sequence/expiry publishing and key rotation/revocation. Complete the selected packs' redistribution materials before publishing their bytes. Keep private signing keys out of the repository, app and public artifacts.
3. Run native source/package acceptance with the completed workflows, capture real UI demos in both themes, update tutorials and the generated skill, and qualify the final artifact. Keep large-pack responsiveness, disk/memory quotas, historical cache retention and power-loss testing in scope.

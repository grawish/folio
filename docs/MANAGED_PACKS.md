...
The native core supports signed catalogs, resumable downloads, authenticated offline archives, deterministic bundle assembly, staged installation and exact repair from retained signed archives. **Settings → LaTeX resources is now connected to the native pack service.** The first publisher identity and reviewed table-pack candidate are prepared, but the ordinary build keeps import/catalog actions disabled until the public assets and catalog are verified. Source-native acceptance uses an ephemeral test publisher; it is not public pack availability. The remaining work below is part of the original release goal.
...
## Publisher preparation and key retirement

The first real publisher key, reproducible source lock, release inventory and bounded renewal command are described in [PACK_PUBLISHING.md](PACK_PUBLISHING.md). Private signing material stays outside the repository. The prepared table pack adds unchanged Multirow 2.9 resources with their complete pinned upstream source/material archives and separate notices. The real native core passed all six offline table cases (A4/US Letter at 10/11/12 points), preserved the included compiler and produced readable inspected PDFs. Evidence: `test-results/published-pack-5yJ0Ad/result.json` and `test-results/published-pack-offline-final.log`. This qualifies the local candidate; public HTTPS and packaged UI acceptance are still separate steps.
...
1. Publish the reviewed pack and all its source/material assets, activate the signed public catalog and configure the ordinary app trust. Verify hosted renewal and actual HTTPS downloads. Keep private signing keys out of the repository, app and public artifacts.
2. Run acceptance against the production catalog and final packaged artifact. The source-native workflow and its test-publisher screenshots do not qualify that future artifact. Keep large-pack responsiveness, disk/memory quotas, historical cache retention and power-loss testing in scope.

...
This is tested on the Apple silicon development Mac. Periodic scanning is not a portable lock against hostile concurrent filesystem mutation, and network filesystems/native Windows/Linux acceptance remain release work. Recovery from an interrupted multi-file save that conflicts with a newer outside edit still needs a separate guided resolution flow; see [save reliability](SAVE_RELIABILITY.md).

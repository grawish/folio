...
4. Controlled template render comparisons, production pack trust/catalog configuration and online repair acceptance. Guided local fonts and the Settings pack workflow are implemented; the latter uses a temporary test publisher in source-native checks. Exact compiler retention, local offline repair and backed-up compiler migration pass their local/native workflows; broader Mac acceptance remains open.
...
The native service now connects authenticated catalogs, reviewed imports, cancel/resume, staged installation, offline retry and cache clearing to Settings → LaTeX resources. A pack changes a project only after a real PDF preview and explicit Apply; failed old builds keep their error instead of an invented before PDF. Apply rechecks the target compiler. The full source suite passes 192 unit/protocol tests. The archive/installation core also passed all 15 real compiler integrations. The source-native pack workflow and existing included-compiler comparison regression pass with zero renderer errors. Three labeled screenshots and a plain-language guide show the development workflow.

The normal build still has no production publisher key or public catalog, so its import/catalog actions are disabled. Tests use an ephemeral build-time public key and simulated HTTPS transport with real compilers and disk operations. Production trust/catalog configuration, licensed distributable packs, public-host checks and final packaged acceptance remain required. See [implementation, evidence and next steps](MANAGED_PACKS.md).

...
## Managed pack protocol in progress

The signed-catalog and resumable-download backend has 15 passing protocol tests, including real network timeout, disk-write failure and three forced process-exit boundaries. The full source suite passes 173 tests. This code is not yet connected to the desktop app; offline archive authentication, publisher configuration, staged runtime installation, Settings workflows, native screenshots and final acceptance remain required. See [the protocol and next steps](MANAGED_PACKS.md).

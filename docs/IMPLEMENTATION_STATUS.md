...
## Managed pack installation core in progress

The native core now authenticates offline archives, builds deterministic resource bundles, stages and tests complete runtimes, and repairs exact packs from retained signed archives. A maintainer build/signing command is available. Fifteen transfer tests and eleven archive/installation tests pass, including time-zone reproducibility and real process kills; the full source suite passes 184 tests. All 15 real compiler integrations pass, including offline use of a newly added package and refusal of a signed pack with a failing probe. Pack APIs are not yet connected to IPC or Settings. Production trust/catalog configuration, Settings workflows, native screenshots and final acceptance remain required. See [the implementation and next steps](MANAGED_PACKS.md).

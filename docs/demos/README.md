...
The resource-pack demos use `npm run test:packs`. The script builds with an ephemeral test public key, simulates the HTTPS catalog/range transport, and uses the real Tectonic/Biber sandbox. It verifies review/discard, cancellation before a request begins, cancel/resume, native close, reload during staging, offline retry, a missing-package preview, explicit Apply, backups and save/restart. Its `finally` block restores the ordinary native build. No private key is written or published. The screenshots are labeled as development workflows with a test publisher; the normal build has no public pack catalog configured yet. The raw compiler error is collapsed in the curated preview.
